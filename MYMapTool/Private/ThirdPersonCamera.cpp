...
void CThirdPersonCamera::Revolution360(_float fTimeDelta)
{
    if (m_fRevolutionAccTime < m_fRevolutionTime)
    {
        ...
        _float fRevolutionAngle = m_fRevolutionAccTime / m_fRevolutionTime * XM_2PI;
...
void CThirdPersonCamera::TiltAdjust(_float fTimeDelta)
{
    ...
    if (m_fTiltTimer <= m_fTiltDuration)
    {
        ...
        m_fRollAngle = LerpFloat(0.f, m_fTargetRollAngle, SmoothStepFloat(t));
    }
    else if (m_fTiltTimer <= m_fTiltDuration + m_fTiltRecoveryDuration)
    {
        // 원래 각도로 복귀
        _float t = (m_fTiltTimer - m_fTiltDuration) / m_fTiltRecoveryDuration;
        m_fRollAngle = LerpFloat(m_fTargetRollAngle, 0.f, SmoothStepFloat(t));
    }
    else
    {
        // Tilt 종료
