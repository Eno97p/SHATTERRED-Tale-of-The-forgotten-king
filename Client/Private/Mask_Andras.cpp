#include "stdafx.h"
#include "..\Public\Mask_Andras.h"

#include "GameInstance.h"
#include "Player.h"


CMask_Andras::CMask_Andras(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CWeapon{ pDevice, pContext }
{
}

CMask_Andras::CMask_Andras(const CMask_Andras& rhs)
	: CWeapon{ rhs }
{
}

HRESULT CMask_Andras::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMask_Andras::Initialize(void* pArg)
{
...
void CMask_Andras::Priority_Tick(_float fTimeDelta)
{
...
void CMask_Andras::Tick(_float fTimeDelta)
{
...
void CMask_Andras::Late_Tick(_float fTimeDelta)
{
...
HRESULT CMask_Andras::Add_Components()
{
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Mask_Andras"),
...
HRESULT CMask_Andras::Bind_ShaderResources()
{
...
CMask_Andras* CMask_Andras::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CMask_Andras* pInstance = new CMask_Andras(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CMask_Andras");
...
CGameObject* CMask_Andras::Clone(void* pArg)
{
	CMask_Andras* pInstance = new CMask_Andras(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CMask_Andras");
		Safe_Release(pInstance);
	}
