#include "stdafx.h"
#include "..\Public\Weapon_Malkhel.h"
...
CWeapon_Malkhel::CWeapon_Malkhel(const CWeapon_Malkhel& rhs)
	: CWeapon{ rhs }
{
}

HRESULT CWeapon_Malkhel::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CWeapon_Malkhel::Initialize(void* pArg)
{
...
void CWeapon_Malkhel::Priority_Tick(_float fTimeDelta)
{
...
void CWeapon_Malkhel::Tick(_float fTimeDelta)
{
...
HRESULT CWeapon_Malkhel::Render()
{
...
HRESULT CWeapon_Malkhel::Render_LightDepth()
{
...
HRESULT CWeapon_Malkhel::Add_Components()
{
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Weapon_Malkhel"),
...
HRESULT CWeapon_Malkhel::Bind_ShaderResources()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", &m_WorldMatrix)))
		return E_FAIL;
...
CWeapon_Malkhel* CWeapon_Malkhel::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CWeapon_Malkhel* pInstance = new CWeapon_Malkhel(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CWeapon_Malkhel");
...
CGameObject* CWeapon_Malkhel::Clone(void* pArg)
{
	CWeapon_Malkhel* pInstance = new CWeapon_Malkhel(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CWeapon_Malkhel");
		Safe_Release(pInstance);
	}
