...
void CHammerSpawn::Tick(_float fTimeDelta)
{
	...
	if (!EffectSpawned && static_cast<Charge_Vane*>(m_EffectClasses[1])->Get_Effect_Dead())
	{
		EFFECTMGR->Generate_Particle(86, m_OwnDesc->vStartPos, nullptr, XMVectorSet(1.f, 0.f, 0.f, 0.f), 90.f);
		EFFECTMGR->Generate_Particle(87, m_OwnDesc->vStartPos);
		EffectSpawned = true;
	}
...
{
	EFFECTMGR->Generate_Particle(60, m_OwnDesc->vStartPos);
