#include "stdafx.h"
#include "..\Public\Weapon_Andras5.h"

#include "GameInstance.h"
#include "Player.h"
#include "Andras.h"

CWeapon_Andras5::CWeapon_Andras5(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CWeapon{ pDevice, pContext }
{
}

CWeapon_Andras5::CWeapon_Andras5(const CWeapon_Andras5& rhs)
	: CWeapon{ rhs }
{
}

HRESULT CWeapon_Andras5::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CWeapon_Andras5::Initialize(void* pArg)
{
...
void CWeapon_Andras5::Priority_Tick(_float fTimeDelta)
{
...
void CWeapon_Andras5::Tick(_float fTimeDelta)
{
...
void CWeapon_Andras5::Late_Tick(_float fTimeDelta)
{
...
HRESULT CWeapon_Andras5::Render()
{
...
void CWeapon_Andras5::Set_Active(_bool isActive)
{
	if (m_bIsActive == false && isActive == true)
	{
		if (*m_pState == CAndras::STATE_SPRINTATTACK)
		{
			...
			m_pGameInstance->Play_Effect_Sound(TEXT("Andras_SprintAttack.ogg"), SOUND_MONSTER);
		}
...
HRESULT CWeapon_Andras5::Add_Components()
{
	/* For.Com_Collider */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Weapon_Andras5"),
...
HRESULT CWeapon_Andras5::Bind_ShaderResources()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", &m_WorldMatrix)))
		return E_FAIL;
...
CWeapon_Andras5* CWeapon_Andras5::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CWeapon_Andras5* pInstance = new CWeapon_Andras5(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CWeapon_Andras5");
...
CGameObject* CWeapon_Andras5::Clone(void* pArg)
{
	CWeapon_Andras5* pInstance = new CWeapon_Andras5(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CWeapon_Andras5");
		Safe_Release(pInstance);
	}
