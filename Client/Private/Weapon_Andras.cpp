#include "stdafx.h"
#include "..\Public\Weapon_Andras.h"
...
CWeapon_Andras::CWeapon_Andras(const CWeapon_Andras& rhs)
	: CWeapon{ rhs }
{
}

HRESULT CWeapon_Andras::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CWeapon_Andras::Initialize(void* pArg)
{
...
void CWeapon_Andras::Priority_Tick(_float fTimeDelta)
{
...
void CWeapon_Andras::Tick(_float fTimeDelta)
{
...
void CWeapon_Andras::Late_Tick(_float fTimeDelta)
{
...
HRESULT CWeapon_Andras::Render_LightDepth()
{
...
void CWeapon_Andras::Set_Active(_bool isActive)
{
...
HRESULT CWeapon_Andras::Add_Components()
{
	/* For.Com_Collider */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Weapon_Andras"),
...
HRESULT CWeapon_Andras::Bind_ShaderResources()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", &m_WorldMatrix)))
		return E_FAIL;
...
CWeapon_Andras* CWeapon_Andras::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CWeapon_Andras* pInstance = new CWeapon_Andras(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CWeapon_Andras");
...
CGameObject* CWeapon_Andras::Clone(void* pArg)
{
	CWeapon_Andras* pInstance = new CWeapon_Andras(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CWeapon_Andras");
		Safe_Release(pInstance);
	}
