#include "stdafx.h"
#include "..\Public\RadamantheSword.h"
#include "GameInstance.h"
#include "Player.h"

CRadamantheSword::CRadamantheSword(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CWeapon{ pDevice, pContext }
{
}

CRadamantheSword::CRadamantheSword(const CRadamantheSword& rhs)
	: CWeapon{ rhs }
{
}

HRESULT CRadamantheSword::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CRadamantheSword::Initialize(void* pArg)
{
...
HRESULT CRadamantheSword::Render()
{
...
HRESULT CRadamantheSword::Render_Reflection()
{
	if (FAILED(Bind_ShaderResources()))
		return E_FAIL;
...
HRESULT CRadamantheSword::Render_LightDepth()
{
...
HRESULT CRadamantheSword::Render_Bloom()
{
	if (FAILED(Bind_ShaderResources()))
		return E_FAIL;
...
void CRadamantheSword::Set_Active(_bool isActive)
{
	if (m_bIsActive == false && isActive == true)
	{
		if (*m_pState != CPlayer::STATE_SPECIALATTACK2)
		{
...
HRESULT CRadamantheSword::Add_Components()
{
	/* For.Com_Collider */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_RadamantheSword"),
...
HRESULT CRadamantheSword::Bind_ShaderResources()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", &m_WorldMatrix)))
		return E_FAIL;
...
#pragma endregion 모션블러
	...
	if (FAILED(m_pShaderCom->Bind_RawValue("g_DisolveValue", &m_fDisolveValue, sizeof(_float))))
		return E_FAIL;
	_float4 fDisolveColor = _float4(1.f, 1.f, 0.5f, 1.f);
...
CRadamantheSword* CRadamantheSword::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CRadamantheSword* pInstance = new CRadamantheSword(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CRadamantheSword");
...
CGameObject* CRadamantheSword::Clone(void* pArg)
{
	CRadamantheSword* pInstance = new CRadamantheSword(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CRadamantheSword");
		Safe_Release(pInstance);
	}
