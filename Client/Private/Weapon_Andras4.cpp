#include "stdafx.h"
#include "..\Public\Weapon_Andras4.h"

#include "GameInstance.h"
#include "Player.h"


CWeapon_Andras4::CWeapon_Andras4(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CWeapon{ pDevice, pContext }
{
}

CWeapon_Andras4::CWeapon_Andras4(const CWeapon_Andras4& rhs)
	: CWeapon{ rhs }
{
}

HRESULT CWeapon_Andras4::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CWeapon_Andras4::Initialize(void* pArg)
{
...
void CWeapon_Andras4::Priority_Tick(_float fTimeDelta)
{
...
void CWeapon_Andras4::Tick(_float fTimeDelta)
{
...
HRESULT CWeapon_Andras4::Render()
{
...
HRESULT CWeapon_Andras4::Render_LightDepth()
{
...
HRESULT CWeapon_Andras4::Add_Components()
{
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Weapon_Andras4"),
...
HRESULT CWeapon_Andras4::Bind_ShaderResources()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", &m_WorldMatrix)))
		return E_FAIL;
...
CWeapon_Andras4* CWeapon_Andras4::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CWeapon_Andras4* pInstance = new CWeapon_Andras4(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CWeapon_Andras4");
...
CGameObject* CWeapon_Andras4::Clone(void* pArg)
{
	CWeapon_Andras4* pInstance = new CWeapon_Andras4(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CWeapon_Andras4");
		Safe_Release(pInstance);
	}
