...
BEGIN(Client)
class Charge_Vane final : public CBlendObject
{
public:
	typedef struct VANE_DESC
	{
...
private:
	Charge_Vane(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	Charge_Vane(const Charge_Vane& rhs);
	virtual ~Charge_Vane() = default;
...
private:
	shared_ptr<VANE_DESC>			m_OwnDesc;
...
public:
	static Charge_Vane* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
