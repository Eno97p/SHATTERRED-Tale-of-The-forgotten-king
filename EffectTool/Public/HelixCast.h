...
BEGIN(Effect)
class CHelixCast final : public CBlendObject
{
public:
	typedef struct HELIXCAST
	{
...
private:
	CHelixCast(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CHelixCast(const CHelixCast& rhs);
	virtual ~CHelixCast() = default;
...
private:
	shared_ptr<HELIXCAST>		m_OwnDesc;
	_float						m_fCurLifeTime = 0.f;
	_float						m_fLifeTimeRatio = 0.f;


public:
	static CHelixCast* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
