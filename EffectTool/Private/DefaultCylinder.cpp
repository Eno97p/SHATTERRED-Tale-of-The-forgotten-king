...
CDefaultCylinder::CDefaultCylinder(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CBlendObject(pDevice, pContext)
{
}

CDefaultCylinder::CDefaultCylinder(const CDefaultCylinder& rhs)
	:CBlendObject(rhs)
{
}

HRESULT CDefaultCylinder::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CDefaultCylinder::Initialize(void* pArg)
{
	if (pArg == nullptr)
		return E_FAIL;
	m_OwnDesc = make_shared<DEFAULTCYLINDER>(*((DEFAULTCYLINDER*)pArg));
...
void CDefaultCylinder::Priority_Tick(_float fTimeDelta)
{
}

void CDefaultCylinder::Tick(_float fTimeDelta)
{
...
void CDefaultCylinder::Late_Tick(_float fTimeDelta)
{
...
HRESULT CDefaultCylinder::Render()
{
...
HRESULT CDefaultCylinder::Render_Bloom()
{
...
HRESULT CDefaultCylinder::Add_Components()
{
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_DefaultCylinder"),
...
HRESULT CDefaultCylinder::Bind_ShaderResources()
{
...
HRESULT CDefaultCylinder::Bind_BloomResources()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", m_pTransformCom->Get_WorldFloat4x4())))
		return E_FAIL;
	if (FAILED(m_pShaderCom->Bind_Matrix("g_ViewMatrix", m_pGameInstance->Get_Transform_float4x4(CPipeLine::D3DTS_VIEW))))
...
CDefaultCylinder* CDefaultCylinder::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CDefaultCylinder* pInstance = new CDefaultCylinder(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CDefaultCylinder");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CDefaultCylinder::Clone(void* pArg)
{
	CDefaultCylinder* pInstance = new CDefaultCylinder(*this);
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CDefaultCylinder");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CDefaultCylinder::Free()
{
	__super::Free();
