...
CHelixCast::CHelixCast(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CBlendObject(pDevice, pContext)
{
}

CHelixCast::CHelixCast(const CHelixCast& rhs)
	:CBlendObject(rhs)
{
}

HRESULT CHelixCast::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CHelixCast::Initialize(void* pArg)
{
	if (pArg == nullptr)
		return E_FAIL;
	m_OwnDesc = make_shared<HELIXCAST>(*((HELIXCAST*)pArg));
...
void CHelixCast::Priority_Tick(_float fTimeDelta)
{

}

void CHelixCast::Tick(_float fTimeDelta)
{
...
void CHelixCast::Late_Tick(_float fTimeDelta)
{
...
HRESULT CHelixCast::Render_Bloom()
{
...
HRESULT CHelixCast::Add_Components()
{
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Andras_HelixCast"),
...
HRESULT CHelixCast::Bind_ShaderResources()
{
...
HRESULT CHelixCast::Bind_BloomResources()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", m_pTransformCom->Get_WorldFloat4x4())))
		return E_FAIL;
	if (FAILED(m_pShaderCom->Bind_Matrix("g_ViewMatrix", m_pGameInstance->Get_Transform_float4x4(CPipeLine::D3DTS_VIEW))))
...
CHelixCast* CHelixCast::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CHelixCast* pInstance = new CHelixCast(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CHelixCast");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CHelixCast::Clone(void* pArg)
{
	CHelixCast* pInstance = new CHelixCast(*this);
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CHelixCast");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CHelixCast::Free()
{
	__super::Free();
