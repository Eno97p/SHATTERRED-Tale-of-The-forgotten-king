...
CTornado_Root::CTornado_Root(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CBlendObject(pDevice, pContext)
{
}

CTornado_Root::CTornado_Root(const CTornado_Root& rhs)
	:CBlendObject(rhs)
{
}

HRESULT CTornado_Root::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTornado_Root::Initialize(void* pArg)
{
	if (pArg == nullptr)
		return E_FAIL;
	m_OwnDesc = make_shared<TORNADO_ROOT>(*((TORNADO_ROOT*)pArg));
	...
	m_CurrentSize = { 0.f,m_OwnDesc->StartYScale, 0.f };
	return S_OK;
}

void CTornado_Root::Priority_Tick(_float fTimeDelta)
{
	if (EffectDead)
		return;
}

void CTornado_Root::Tick(_float fTimeDelta)
{
	if (EffectDead)
		return;
	...
	CurSize = XMVectorLerp(CurSize, LerpSize, fTimeDelta);
...
void CTornado_Root::Late_Tick(_float fTimeDelta)
{
	if (EffectDead)
		return;
...
HRESULT CTornado_Root::Render_Distortion()
{
	if (FAILED(Bind_ShaderResources()))
		return E_FAIL;
	_uint	iNumMeshes = m_pModelCom->Get_NumMeshes();
...
HRESULT CTornado_Root::Add_Components()
{
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_TornadoBottom"),
...
HRESULT CTornado_Root::Bind_ShaderResources()
{
...
CTornado_Root* CTornado_Root::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CTornado_Root* pInstance = new CTornado_Root(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CTornado_Root");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CTornado_Root::Clone(void* pArg)
{
	CTornado_Root* pInstance = new CTornado_Root(*this);
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CTornado_Root");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CTornado_Root::Free()
{
	__super::Free();
