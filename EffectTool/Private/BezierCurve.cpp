...
CBezierCurve::CBezierCurve(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CBlendObject(pDevice, pContext)
{
}

CBezierCurve::CBezierCurve(const CBezierCurve& rhs)
	:CBlendObject(rhs)
{
}

HRESULT CBezierCurve::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBezierCurve::Initialize(void* pArg)
{
	if (pArg == nullptr)
		return E_FAIL;
	m_OwnDesc = make_shared<BEZIER_CURVE>(*((BEZIER_CURVE*)pArg));
...
void CBezierCurve::Priority_Tick(_float fTimeDelta)
{

}

void CBezierCurve::Tick(_float fTimeDelta)
{
...
void CBezierCurve::Late_Tick(_float fTimeDelta)
{
...
HRESULT CBezierCurve::Render_Bloom()
{
...
HRESULT CBezierCurve::Add_Components()
{
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_BezierCurve"),
...
HRESULT CBezierCurve::Bind_ShaderResources()
{
...
HRESULT CBezierCurve::Bind_BloomResources()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", m_pTransformCom->Get_WorldFloat4x4())))
		return E_FAIL;
	if (FAILED(m_pShaderCom->Bind_Matrix("g_ViewMatrix", m_pGameInstance->Get_Transform_float4x4(CPipeLine::D3DTS_VIEW))))
...
CBezierCurve* CBezierCurve::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CBezierCurve* pInstance = new CBezierCurve(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CBezierCurve");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CBezierCurve::Clone(void* pArg)
{
	CBezierCurve* pInstance = new CBezierCurve(*this);
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CBezierCurve");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CBezierCurve::Free()
{
	__super::Free();
