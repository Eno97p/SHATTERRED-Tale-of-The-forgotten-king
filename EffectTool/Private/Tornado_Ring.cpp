...
void CTornado_Ring::Tick(_float fTimeDelta)
{
	...
	if (m_OwnDesc->StartWithBody)
	{
		_vector vPos = XMVectorLerp(m_pTransformCom->Get_State(CTransform::STATE_POSITION), vFinalPos, m_OwnDesc->fGrowSpeed * fTimeDelta);
...
{
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_TornadoRing"),
